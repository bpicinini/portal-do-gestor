    ...
    if fd == "Todos":
        ...
        with tab_org:
            ...
            html_doc = (
                ...
                f"html{{overflow:auto;scrollbar-width:none;-ms-overflow-style:none;}}"
                f"html::-webkit-scrollbar{{display:none;}}"
                f"body{{background:transparent;font-family:system-ui,-apple-system,sans-serif;"
                f"padding:16px 24px 28px;overflow:visible;text-align:center;cursor:grab;user-select:none;}}"
                f"body.dragging{{cursor:grabbing;}}"
                ...
                f"#oc-toolbar{{position:fixed;top:8px;right:10px;display:flex;gap:6px;z-index:999;}}"
                f"#oc-toolbar button{{background:{'rgba(30,50,70,0.85)' if _dark else 'rgba(240,245,250,0.95)'};"
                f"color:{_text_main};border:1px solid {_card_bdr};border-radius:5px;"
                f"padding:4px 10px;font-size:11px;cursor:pointer;font-family:system-ui,sans-serif;}}"
                f"#oc-toolbar button:hover{{opacity:0.8;}}"
                f"</style>"
                f"<script src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js'></script>"
                f"</head><body>"
                f"<div id='oc-toolbar'>"
                f"<button id='btn-dl' onclick='downloadImg()'>&#11015; Baixar PNG</button>"
                f"<button id='btn-fs' onclick='toggleFS()'>&#x26F6; Tela cheia</button>"
                f"</div>"
                f"<div class='oc-wrap-inner'><ul class='oc-root'>{inner}</ul></div>"
                f"<script>"
                f"function downloadImg(){{"
                f"  var btn=document.getElementById('btn-dl');"
                f"  btn.textContent='Gerando...';btn.disabled=true;"
                f"  var el=document.querySelector('.oc-wrap-inner');"
                f"  html2canvas(el,{{backgroundColor:'{_bg_body}',scale:2,useCORS:true,logging:false,"
                f"    width:el.scrollWidth,height:el.scrollHeight}}).then(function(c){{"
                f"    var a=document.createElement('a');"
                f"    a.download='organograma.png';a.href=c.toDataURL('image/png');a.click();"
                f"    btn.textContent='\\u2b07 Baixar PNG';btn.disabled=false;"
                f"  }});"
                f"}}"
                f"function toggleFS(){{"
                f"  var el=document.documentElement;"
                f"  var req=el.requestFullscreen||el.webkitRequestFullscreen||el.mozRequestFullScreen;"
                f"  if(!document.fullscreenElement&&!document.webkitFullscreenElement){{"
                f"    if(req)req.call(el);"
                f"  }}else{{"
                f"    var ex=document.exitFullscreen||document.webkitExitFullscreen;"
                f"    if(ex)ex.call(document);"
                f"  }}"
                f"}}"
                ...
                f"}});"
                f"(function(){{"
                f"  var root=document.documentElement,drag=false,sx=0,sy=0,sl=0,st=0;"
                f"  function start(cx,cy){{"
                f"    drag=true;sx=cx;sy=cy;sl=root.scrollLeft;st=root.scrollTop;"
                f"    document.body.classList.add('dragging');"
                f"  }}"
                f"  function move(cx,cy){{"
                f"    if(!drag)return;"
                f"    root.scrollLeft=sl-(cx-sx);"
                f"    root.scrollTop=st-(cy-sy);"
                f"  }}"
                f"  function end(){{drag=false;document.body.classList.remove('dragging');}}"
                f"  document.addEventListener('mousedown',function(e){{"
                f"    if(e.target.closest('#oc-toolbar'))return;"
                f"    start(e.clientX,e.clientY);e.preventDefault();"
                f"  }});"
                f"  document.addEventListener('mousemove',function(e){{move(e.clientX,e.clientY);}});"
                f"  document.addEventListener('mouseup',end);"
                f"  document.addEventListener('mouseleave',end);"
                f"  document.addEventListener('touchstart',function(e){{"
                f"    if(e.target.closest('#oc-toolbar'))return;"
                f"    var t=e.touches[0];start(t.clientX,t.clientY);"
                f"  }},{{passive:true}});"
                f"  document.addEventListener('touchmove',function(e){{"
                f"    if(!drag)return;var t=e.touches[0];move(t.clientX,t.clientY);e.preventDefault();"
                f"  }},{{passive:false}});"
                f"  document.addEventListener('touchend',end);"
                f"}})();"
                f"</script>"
                f"</body></html>"
            )
